...
def distance_to_line(p0):
    ...
    # p1 and p2 points define the m-line
...
def main():
    ...
    sub_laser = rospy.Subscriber('/b00885866Bot/laser/scan', LaserScan, clbk_laser)
    sub_odom = rospy.Subscriber('/odom', Odometry, clbk_odom)

 	#You call a service by creating an instance of rospy.ServiceProxy with the name of the service you wish to call
    rospy.wait_for_service('/go_to_point_switch')
    rospy.wait_for_service('/wall_follower_switch')
    rospy.wait_for_service('/gazebo/set_model_state')
	#used to activate/deactivate go to point service
    srv_client_go_to_point_ = rospy.ServiceProxy('/go_to_point_switch', SetBool)
	#used to activate/deactivate wall follower service
    srv_client_wall_follower_ = rospy.ServiceProxy('/wall_follower_switch', SetBool)
	#Used to set model state without Gazebo
    srv_client_set_model_state = rospy.ServiceProxy('/gazebo/set_model_state', SetModelState)

    # set robot position
    model_state = ModelState()
    model_state.model_name = 'b00885866Bot'
